...
plots_list <-   imap(.x=df_noDup_list, .f = ~(ggdraw()+
  draw_plot(image_gg) +
  annotate("text", x=.65, y=.225, label = unique(.$Site), size= 3, color ="black", hjust = 0) +
  annotate("text", x=.65, y=.205, label = paste("Time-Series Plot for ", unique(.$Plot)),  size= 3, color ="black", hjust = 0,
           fontface = "bold") +

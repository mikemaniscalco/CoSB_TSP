...
# panel.border = element_rect(color = "grey5", fill = NA),
    plot.margin = unit(c(0.1, 5.5, 0.1, 5.5), "pt"),
...
### Make an example of a plot

```{r sample_plot}
df_noDup_list <-df_no_dupes %>%
  split( df_no_dupes , f = df_no_dupes$Analyte)

plots <- df_noDup_list %>%
  map(~ggplot(data=.x, aes(x=`Date Sampled`, y=SigFigResult, group=Loc)) +
        scale_y_continuous(trans = 'log10',breaks = prettyLogs)+
        annotation_logticks(sides = "l",scaled = T, long=unit(5,"mm"))+
        scale_shape_manual(values=c(1,19))+
        ylab(expression("Concentration ("*mu*"g/l)"))+
        scale_x_date(date_labels =  "%b %Y",
                     breaks=seq(as.Date("1988-01-01"), as.Date(max(df_no_dupes$`Date Sampled`)), by="12 months")) +
        geom_line(aes(color=Loc, linetype=Loc),
                  size=0.75)+
        geom_point(aes(color=Loc, shape = Detect
                        ), size=3, alpha = 0.5))
plots$`1,1,1,2-Tetrachloroethane`
```
...
plots_list <- df_no_dupes %>%
  groupsplit(., f = Analyte) %>%
  imap(.x=df_noDup_list, .f = ~(ggdraw()+
  draw_plot(image_gg) +
  annotate("text", x=.65, y=.225, label = figure_site, size= 3, color ="black", hjust = 0) +
  annotate("text", x=.65, y=.205, label = paste("Time-Series Plot for ", unique(.x$Analyte)),  size= 3, color ="black", hjust = 0,
           fontface = "bold") +
  annotate("text", x=.89, y=.175, label = stringr::str_wrap(figure_number, 6), size= 4, color ="black", hjust = 0,  vjust = 1)+
  ...
  annotate("text", x=0.0475, y=0.085, label = figure_file_path, size= 1.5, color ="black",angle=90, hjust = 0) +
    draw_plot(ggplot(data=.x, aes(x=`Date Sampled`, y=SigFigResult, group=Loc)) +
              ...
              x = 0.075, y = 0.15, width = 0.85, height = 0.7)))

# Print one plot for us
plots_list$`1,1-dichloro-1-Propene`
...
```{r ggsave}
# the next line is to only print a small subset of the plots during testing phase
template_plots_30 <- plots_list[2:5]

pdf("all.pdf",width = 11, 
       height = 8.5)
invisible(lapply(template_plots_30, print))
dev.off()
getwd()
```